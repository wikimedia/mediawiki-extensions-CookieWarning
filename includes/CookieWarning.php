...
class CookieWarning {
	...
	public static function isAllowedCookie( string $cookieName, WebRequest $request, User $user ): bool {
		...
		if ( $preferences['preference'] === true &&
			in_array( $name, $config->get( 'CookieWarningPreferenceCookies' ), true ) ) {
			return true;
		}

		if ( $preferences['statistic'] === true &&
			in_array( $name, $config->get( 'CookieWarningStatisticCookies' ), true ) ) {
			return true;
		}

		if ( $preferences['marketing'] === true &&
			in_array( $name, $config->get( 'CookieWarningMarketingCookies' ), true ) ) {
			return true;
		}
